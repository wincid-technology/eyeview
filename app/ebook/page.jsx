...
import { ebooks } from "@/constant/ebook";
...
const BookStore = () => {
  ...
  const languages = [
    "All",
    ...new Set(ebooks.map((b) => b.language).filter(Boolean)),
  ];
  ...
  const filteredBooks = useMemo(() => {
    return ebooks.filter((book) => {
      ...
      const matchesLanguage =
        activeLanguage === "All" ||
        book.language?.toLowerCase() === activeLanguage.toLowerCase();
          ...
          {filteredBooks.length ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {filteredBooks.map((book) => (
                <Link key={book.id} href={`/ebook/${book.slug}`}>
            ...
            <div className="text-center py-20">
              ...
              <p className="text-gray-500">
                Try changing search, category, or language.
              </p>
            </div>
          )}
        </div>
